...
|---|---|---|
| 09-15 | R2-SYNC-SPEC §3.1 — R2 works copy back on live: 565 works synced and checksum-verified (6,656 files; nine Book of Concord works were missing, page shards stale in about half of a sample); `r2_works_sync.py --no-scans` archives everything but the scans; embed manifest refreshed (1,626 works) | mo-ghost-theme (ask-port-ui + docs branch) |
| 09-15 | R2-SYNC-SPEC §5 + §3.1 — the Vectorize index follows Upstash through `tools/vec_pipe.py`, not R2 and not the embed manifest; all six VECTOR2 namespaces re-mirrored (2,774,567 vectors, Scaliger + PG English-lane + de-duplicated units now searchable on MereO); nothing keeps it current automatically | mo-ghost-theme (ask-port-ui + docs branch) |
...
| 09-15 | ASK-SPEC §6.5 — rewrite turns re-render the earlier answer (retrieval on the original question, all its pages carried); quotations in English with the Latin only beside its English, enforced by a deterministic check + one repair round. |
