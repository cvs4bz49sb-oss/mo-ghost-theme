...
| mo-forms | `/zoom-register` | POST | None | Origin allowlist + per-IP RL 5/15min + per-email RL 5/hr (salted hash key) + Turnstile **fail-CLOSED** (503 if `TURNSTILE_SECRET` unset) + name allowlist + WN_-codes-only, matched exactly against `registration_url`'s last path segment + account-wide scan budget 20/hr | **SHIPPED 2026-08-11**, 503s until the `ZOOM_*` secrets are set. Registers a visitor for the /forum/ event's Zoom webinar. Fail-open Turnstile is deliberately NOT used here: every accepted call makes Zoom send mail to a caller-chosen address. Numeric webinar ids are rejected on purpose; a WN_ code is already the public credential for registering, a numeric id is not. Residual: anyone holding a WN_ code for an unannounced webinar on the account could register through this route, exactly as they could through Zoom's own page. |
