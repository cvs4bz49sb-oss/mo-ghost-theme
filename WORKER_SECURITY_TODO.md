...
| mo-membership | `/api/student/checkout` | GET | Signed 1h token (proves inbox control) | Verify token, re-validate `.edu`, re-derive price from `variant → STRIPE_PRICE_STUDENT_*`, `customer_email` locked, **one-time payment** (302 to Stripe); light per-IP RL | NEW 2026-06-22 |
| mo-membership | `/api/student/me` | GET | JWT | Identity from `sub` only; returns caller's own student status (expiry/needs_renewal) for the dashboard renewal banner | NEW 2026-06-22 |
...
| mo-admin | `/migration/*` | GET/POST | Ghost staff JWT, **staff only** | `/admin/migration/` backend: the double-billing queue, the refund ladder, and the only route in the estate that cancels a live HubSpot subscription. `toolForRoute` returns `__staff_only__`, so it cannot be delegated through the admin-permissions map the way `liturgy` or `kpi` can — it reads member email addresses against payment amounts. Dispatched **above** the GET-only gate (its writes are POSTs) but **below** `verifyMemberToken` → `isStaff`. `POST /migration/cancel` re-reads Ghost server-side and refuses when the member is not paid/comped there, refuses again when Ghost is unreachable, and reports `verified` only after re-reading the subscription back from HubSpot — never from the request having succeeded. No refund route exists: HubSpot has no refund API. | NEW 2026-08-11 |
| mo-email | `/health` | GET | None | Liveness only, no state, no Kit call | OK |
| mo-email | `/kit/meta`, `/kit/broadcasts` | GET | Ghost staff JWT | `verifyMemberToken` + `isStaff`, same two-step as mo-admin, above every route. Reads Kit email templates / tags / segments and this worker's own KV push records. Meta cached 5 min. | NEW 2026-07-28 |
| mo-email | `/kit/push`, `/kit/test`, `/kit/unschedule`, `/kit/detach` | POST | Ghost staff JWT | Email Builder → Kit broadcasts. Staff check + Origin allowlist + 30 writes/10min/staffer (`mo-email:staff:<email>`). Broadcasts are created as drafts unless `sendAt` is supplied; nothing sends immediately. `buildSubscriberFilter` throws on an empty audience, so a missing filter can never degrade into a send to the whole list; the everyone-audience needs an explicit `confirmEveryone`, and `subscriber_filter` is always sent explicitly (Kit's docs say an omitted filter defaults to ALL subscribers). The broadcast id to update is read from this worker's own KV record keyed by `draftId`, never from the request body — a body id would let the route rewrite any broadcast the shared Kit key can reach, including mo-kit's scheduled Daily Liturgy sends. `public` is hardcoded false so caller-supplied HTML is never published as a Kit web page. Test sends go only to the caller's own JWT-derived address, scoped to the `mo-email-test` tag. | NEW 2026-07-28 |
...
| mo-membership | `/api/admin/events` | GET | JWT (staff) | `assertAdmin` first, above every D1 and Ghost call: `verifyMemberToken` → `isStaff` against the live Ghost staff list, which fails closed when Ghost is unreachable. Staff-only and **not** delegable through the `admin-permissions` KV tools map, on the same reasoning as `/migration/*` — it returns per-person rows (name, email, membership status and join date) for every `/forum/` registrant, not aggregates. Reads `forum_registrations`, the one table in this DB written by an unauthenticated public endpoint, so **every string in the response is visitor-supplied and untrusted at every sink**: the theme builds the table with `createElement`/`textContent` only, `mailto:` hrefs are rendered only for addresses matching an allowlist (a scheme-literal prefix makes an href inert as markup but not as a command — `?bcc=` is a real exfil vector), and `?format=csv` shares the same auth gate and routes every cell through the shared `csv()` neutraliser. `join_url` is read but deliberately never returned — a Zoom join URL is a seat credential. Bounded at 5000 rows; Ghost enrichment is allowlisted, chunked 4-at-a-time, and memoised per-isolate for 60s behind a single-flight promise, read only after the staff check. Responses are `no-store`. | **NEW 2026-08-14** |
| mo-forms | `/forum-register` (alias `/zoom-register`) | POST | None | Origin allowlist + per-IP RL 5/15min + per-email RL 5/hr (salted hash key) + Turnstile **fail-CLOSED** (503 if `TURNSTILE_SECRET` unset) + name allowlist + WN_-codes-only, matched exactly against `registration_url`'s last path segment + account-wide Zoom scan budget 20/hr | **SHIPPED 2026-08-11.** Takes a /forum/ registration: **writes a row to D1 `forum_registrations` first**, then books the Zoom webinar seat only if the event post carries a WN_ link AND the `ZOOM_*` secrets are set (they are not yet, so today it is record-only and returns `{ok:true, zoom:false}`). Two consequences for review: this route now **persists caller-supplied PII** (name, email, event title/slug) into the shared mo-membership DB, and it is the only unauthenticated write path into that table; and fail-open Turnstile is deliberately NOT used here because an accepted call can make Zoom send mail to a caller-chosen address. Numeric webinar ids are rejected on purpose; a WN_ code is already the public credential for registering, a numeric id is not. Residual: anyone holding a WN_ code for an unannounced webinar on the account could register through this route, exactly as they could through Zoom's own page. |
