...
async function room(slug,arg){
  ...
  <div class="ridbar">
    ...
    <div class="stats"><b>${d.n_works}</b> works · <b>${d.n_pages.toLocaleString()}</b> pages ·
      ...
      · ${esc(RX.shelves[RSH])} · <a href="/the-faith-received/connections/#a=${encodeURIComponent(slug)}">Explore citation map</a> · <a href="${cdURL({a:[slug],sel:'',g:'work',s:'',q:''})}">Compare with other authors</a></div>
  </div>
...
async function topicPage(slug){
 ...
 page.innerHTML=`<div class="crumbs"><a id="topic-back" href="${RX.topicURL('',shelf)}">Browse ${shelf?esc(RX.shelves[shelf])+' topics':'topics'}</a></div><div class="rx-topic-heading"><h1>${esc(d.t)}</h1><a class="rx-text-link" href="/the-faith-received/connections/#t=${encodeURIComponent(d.s||slug)}">View citation map</a></div>
