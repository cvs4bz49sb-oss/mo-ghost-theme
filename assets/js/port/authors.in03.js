...
async function room(slug,arg){
  ...
  page.innerHTML=`
  <div class="crumbs"><a href="/the-faith-received/author/">Authors</a> · <a href="/the-faith-received/author/?sh=${encodeURIComponent(RSH)}">${esc(RX.shelves[RSH])}</a></div>
  <div class="ridbar">
    <h1>${esc(aName(d.a))}</h1>
  ...
  async function renderTopic(t){
    ...
    pbody.innerHTML=`<div class="view"><div class="rx-section-head"><h2>${esc(t.t)}</h2>${globalTopic?`<a class="rx-text-link" href="/the-faith-received/doctrines/?s=${encodeURIComponent(globalTopic.s)}">Compare across authors</a>`:''}</div><p class="pane-meta">${fmtR(t.np)} indexed pages · ${fmtR(t.npos)} recorded positions · ${fmtR(records.length)} available entries</p><p class="rx-note">Mined excerpts and page annotations. Open the source to check its speaker, context, and translation.${t.full&&!full?' The larger export could not load; the room’s sample is shown.':''}</p>
...
async function workPage(dnum){
  ...
  ${ins&&ins.lemma?(()=>{const bn2=BNAME[ins.lemma.b]||ins.lemma.b;const bs2=bn2.toLowerCase().replace(/^([i]{1,3}) /,(m2,r2)=>r2+"-").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");
    return `<div class="pane-meta" style="margin:.3rem 0 .1rem">A commentary on <a href="/bible/#b/${bs2}${ins.lemma.c?"/"+ins.lemma.c:""}">${esc(bn2)}${ins.lemma.c?" "+ins.lemma.c:""}</a> — its chapter holds this work among the commentators.</div>`;})():""}
...
async function bookPageMain(bslug,c){
  ...
  const B=bk.books.find(b=>b.slug===bslug);
  if(!B||c&&!B.chapters.some(x=>x.c===c)){page.innerHTML='<h1>Passage not found</h1><p>Choose an available book and chapter.</p><a href="/bible/">Browse Scripture</a>';return;}
  ...
  page.innerHTML=`<div class="crumbs"><a href="/bible/">Scripture</a>${c?` · <a href="#b/${bslug}">${esc(B.book)}</a>`:''}</div><h1 class="headline">${esc(B.book)}${c?' '+c:''}</h1>${bibleNav(bk.books,B,c)}${scriptureFacets()}${!c?`<section class="bible-chapter-grid" aria-label="Choose a chapter"><h2>Choose a chapter</h2><div>${B.chapters.map(x=>`<a href="#b/${bslug}/${x.c}" aria-label="${esc(B.book)} ${x.c}">${x.c}</a>`).join('')}</div></section>`:''}<nav class="scripture-tabs" aria-label="Chapter views">${tabs.map(([k,label])=>`<button data-bible-view="${k}" aria-pressed="${view===k}">${label}${k==='read'||k==='desk'?'':` <span>${k==='annotations'?annotations.length:dedicated.length}</span>`}</button>`).join('')}</nav>
...
async function comparePage(){
  ...
  page.innerHTML=`<div class="crumbs"><a href="/the-faith-received/doctrines/">Topics</a> · <a href="/the-faith-received/author/">Authors</a></div>
...
function route(){
  ...
  if(h==="t"||h.startsWith("t/")){location.replace("/the-faith-received/doctrines/"+(h==="t"?"":"#"+h.slice(2)));return;}
  ...
  if(PAGE==="bible"){
    ...
    const token=BIBLE_RUN;task.catch(()=>{if(token!==BIBLE_RUN)return;page.innerHTML='<h1>Scripture could not load</h1><p>Your passage is still in the address bar.</p><button id="retry-bible">Retry loading</button> <a href="/bible/">Browse Scripture</a>';$("#retry-bible").onclick=route;});
    return;
  }
