...
const RX=FRResearch,fmtR=n=>Number(n||0).toLocaleString();if(window.MODenom)window.MODenom.ready();   // church labels for single authors (compare voices, author header)
...
async function authorsIndex(){
 ...
 const rowHTML=r=>`<article class="rx-author-row"><a class="rx-author-main" href="${RX.authorURL(r)}"><span><strong>${esc(aName(r.a))}</strong><small>${r.y?'c. '+r.y+' · ':''}${esc(RX.eras[r.e])}</small></span><span class="rx-counts"><span>${fmtR(r.w)} ${r.w===1?'work':'works'}</span><span>${fmtR(r.nt)} topic entries</span>${AV.sort==='n'?`<span>${fmtR(r.n)} Scripture citations</span>`:''}</span></a><div class="rx-author-actions"><a class="rx-text-link" href="${RX.authorURL(r)}">Browse works</a><a class="rx-text-link" href="${RX.authorURL(r)}/positions">Browse positions</a></div>${r.variants.length>1?`<div class="rx-editions">Also on ${r.variants.slice(1).map(v=>`<a href="${RX.authorURL(v)}">${esc(RX.shelfName(v.sh))} (${fmtR(v.w)} works)</a>`).join(' · ')}</div>`:''}</article>`;
 page.innerHTML=`<div class="research-intro"><div><h1>Authors</h1><p>Every writer in the library, shelf by shelf. Open a tradition to see its authors; open an author to read their works, follow their use of Scripture, and explore the passages behind their ideas.</p></div><a class="rx-text-link" href="/the-faith-received/connections/">Explore citation connections</a></div>
 ...
 ${index.missing.length?`<p class="rx-note">${index.missing.map(k=>RX.shelfName(k)).join(', ')} could not load. <button id="roster-retry" class="rx-text-link">Retry shelves</button></p>`:''}
 <div id="dir" class="rx-fold-list rx-author-folds">${ORDER.map(sh=>`<details class="rx-fold rx-author-fold" data-sh="${sh}"${sh===shelfOpen?' open':''}><summary><span><strong>${esc(RX.shelfName(sh))}</strong><small class="rx-fold-count"></small></span><span class="rx-fold-preview"></span></summary><div class="rx-fold-body"><div class="rx-directory"></div><button class="rx-button rx-more" hidden>Show all</button></div></details>`).join('')}</div>
 ...
 const paint=()=>{const cmp=sorter(),reveal=AV.q!==revealedQuery;revealedQuery=AV.q;let total=0;
  ...
  const shelves=ORDER.filter(sh=>byShelf[sh].filter(matches).length).length;$('#dircount').textContent=fmtR(total)+(total===1?' author across ':' authors across ')+fmtR(shelves)+(shelves===1?' shelf':' shelves');const visible=[...document.querySelectorAll('.rx-author-fold')].filter(d=>!d.hidden);$('#author-expand').textContent=visible.some(d=>!d.open)?'Expand all':'Collapse all';$('#author-expand').disabled=!visible.length;
...
// The theme's own index (collected-work-contents.json through window.MOCollectedContents, as the rooms read it) when the page
// loads it; else v1/collected-contents.json, the same file published.
let COLLECTED={},COLLECTED_P=null;
const collectedOf=slug=>(window.MOCollectedContents&&window.MOCollectedContents.get(slug))||COLLECTED[slug];
const collectedReady=()=>COLLECTED_P||(COLLECTED_P=window.MOCollectedContents?window.MOCollectedContents.ready:J(BLOB+'/v1/collected-contents.json').then(d=>{COLLECTED=(d&&d.works)||{};}).catch(()=>{}));
const collectedText=slug=>{const c=collectedOf(slug);return c?[c.summary,...(c.sections||[]).map(x=>x.title)].join(' '):'';};
...
async function topicsIndex(){
  ...
  page.innerHTML=`<div class="research-intro"><div><h1>Topics</h1><p>Explore a theological question through its sources. Find passages, compare authors, and follow the argument into the text.</p></div></div>
  <div class="rx-filters"><label class="rx-search">Find a topic<input type="search" id="q" placeholder="Grace, Trinity, justification" value="${esc(TV.q)}"></label><label class="rx-primary-filter">Shelf<select id="topics-shelf"><option value="">All shelves</option>${Object.keys(RX.shelves).map(k=>`<option value="${k}">${esc(RX.shelfName(k))}</option>`).join('')}</select></label><label>Order topics<select id="sort"><option value="group">Theological subjects</option><option value="pages">Most indexed pages</option><option value="a">Name A–Z</option></select></label></div><div class="rx-results-line"><p role="status"><span id="dircount"></span> topics${shelf?' in '+esc(RX.shelfName(shelf)):' across all shelves'}</p><span><button class="rx-text-link" id="topics-collapse">Collapse all</button> <button class="rx-text-link" id="topics-reset">Reset filters</button></span></div><p class="rx-note">Open a subject to browse its topics. ${shelf?'Counts describe indexed pages on this shelf. <a href="/the-faith-received/connections/#shelves='+RX.shelfMaps[shelf]+'/doctrines">Explore this shelf’s full topic map</a>.':'Counts describe the full index; each topic links to its available evidence.'}</p><div class="dir rx-fold-list" id="dir"></div>`;
...
async function room(slug,arg){
  ...
  // One author's label names his church once the denomination table lands (Keach: Baptist, not "English Divines").
  if(window.MODenom)window.MODenom.ready().then(()=>{if(run!==RESEARCH_RUN)return;const el=page.querySelector('[data-author-church]');if(el)el.textContent=RX.shelfName(RSH,d.a);});
  page.innerHTML=`
  <div class="crumbs"><a href="/the-faith-received/author/">Authors</a> · <a href="/the-faith-received/author/?sh=${encodeURIComponent(RSH)}">${esc(RX.shelfName(RSH))}</a></div>
  <div class="ridbar">
    <h1>${esc(aName(d.a))}</h1>
    ...
    <div class="stats"><b>${d.n_works}</b> works · <b>${d.n_pages.toLocaleString()}</b> pages ·
      ...
      · <span data-author-church>${esc(RX.shelfName(RSH,d.a))}</span> · <a href="/the-faith-received/connections/#a=${encodeURIComponent(slug)}">Explore citation map</a> · <a href="${cdURL({a:[slug],sel:'',g:'work',s:'',q:''})}">Compare with other authors</a></div>
  </div>
  ${candidates.length>1?`<p class="rx-note">Available shelves: ${candidates.map(r=>`<a href="${RX.authorURL(r)}"${r.sh===RSH?' aria-current="page"':''}>${esc(RX.shelfName(r.sh))}</a>`).join(" · ")}</p>`:""}
  ...
  function renderPositions(topicName){
    ...
    pbody.innerHTML=`<div class="view rx-positions"><h2>Positions</h2><p class="pane-meta">Mined statements with their source passages, in the order of the loci. Open a topic and read; the index fills in as you scroll.</p><p class="rx-note rx-paraphrase-note">These are machine-written summaries of what each passage says, not the author&rsquo;s words. No quotation on this page is verbatim. Open the source to read the passage itself, and to check its speaker, context and translation.</p>
  ...
  function renderConnections(){
    ...
    const showPair=p=>{sel=p;$('#room-connections').querySelectorAll('[data-pair]').forEach(b=>{const on=+b.dataset.pair===pairs.indexOf(p);b.classList.toggle('on',on);b.setAttribute('aria-selected',String(on));});
      ...
      $('#connection-detail').innerHTML=`<h3 class="rx-conn-title">${esc(p.a)} <span class="rx-join">with</span> ${esc(p.b)}<small>${fmtR(p.pages.length)} shared ${p.pages.length===1?'page':'pages'} across ${fmtR(works.length)} ${works.length===1?'work':'works'}</small></h3><div class="rx-pane rx-conn-pane">${works.map(([w,rs],i)=>`<details class="cd-work"${i===0?' open':''}><summary><span><strong>${esc(title(rs[0]))}</strong></span><small class="cd-n">${fmtR(rs.length)}</small> ${foldOpen(w,rs)}${workSaveBtn(w,title(rs[0]),d.a)}</summary><div class="cd-fold-body">${sectionFoldsHTML(rs,r=>`<article class="rx-excerpt"><p>${esc(r.q||r.g||'Topics recorded together on this page.')}</p><div class="rx-source"><span>${esc(title(r))} · ${pgl(r.w)} ${esc(r.p??'')}</span><div><a class="rx-text-link" href="${readerHref(r.w,r.p)}">Read shared passage</a>${previewBtn(r.w,r.p)}${pinBtn(r.w,r.p,title(r),d.a,r.q||r.g)}</div></div></article>`,w)}</div></details>`).join('')}</div>`;};
  ...
  function renderSearch(){
    ...
    const local=()=>{
      ...
      if(posH.length)out.push('<h2 class="sect">Recorded statements</h2><p class="rx-note rx-paraphrase-note">These are machine-written summaries of what each passage says, not the author&rsquo;s words. No quotation on this page is verbatim. Open the source to read the passage itself, and to check its speaker, context and translation.</p>'+workFoldsHTML(posH.map(({t,p})=>({...p,topicLabel:t.t})),{title:titleOf,author:d.a,annotation:true,extra:r=>'<p class="rx-note">'+esc(r.topicLabel)+'</p>'}));
  ...
  (async()=>{try{
    ...
    $("#wauth").innerHTML=`<h2 class="sect">Its authorities <span class="tn" style="font-family:var(--body);font-size:.74rem;color:var(--faint)">${tot.toLocaleString()} resolved citations of ${mine.length} authors</span></h2>`+
      mine.slice(0,40).map((g,i)=>`<details class="wdet"${i===0?" open":""}><summary><b>${esc(g.a)}</b><i style="width:${Math.max(3,40*Math.sqrt(g.rows.length/mine[0].rows.length)).toFixed(0)}%"></i><span class="n">${g.rows.length}</span></summary>
        ...
        ${g.rows.length>80?`<div style="color:var(--faint);font-size:.78rem;margin-top:.3rem">+ ${(g.rows.length-80).toLocaleString()} more in <a href="/the-faith-received/author/#${aslug(A)}/reception">${esc(A)}’s Reception</a></div>`:""}</div></details>`).join("");
  }catch(_){}})();
...
async function topicPage(slug){
 ...
 page.innerHTML=`<div class="crumbs"><a id="topic-back" href="${RX.topicURL('',shelf)}">Browse ${shelf?esc(RX.shelfName(shelf))+' topics':'topics'}</a></div><div class="rx-topic-heading"><h1>${esc(d.t)}</h1><a class="rx-text-link" href="/the-faith-received/connections/#t=${encodeURIComponent(d.s||slug)}">View citation map</a></div>
 ...
 <div class="rx-filters"><label class="rx-search">Search available evidence<input id="tq" type="search" placeholder="Author, work, or phrase"></label><label class="rx-primary-filter">Shelf<select id="topic-shelf"><option value="">All shelves</option>${Object.keys(RX.shelves).map(k=>`<option value="${k}">${esc(RX.shelfName(k))}</option>`).join('')}<option value="unknown">Shelf not recorded</option></select></label><label>Order authors<select id="topic-order"><option value="pages">Available passages first</option><option value="name">Name A–Z</option><option value="date">Earlier authors first</option></select></label></div>
 <div class="rx-results-line"><p id="topic-count" role="status"></p><div class="rx-result-actions"><button class="rx-text-link" id="topic-collapse">Collapse all</button><button class="rx-text-link" id="topic-reset">Reset filters</button></div></div><p class="rx-note" id="compare-help">Select up to three authors to compare their available passages.</p><p class="rx-note rx-paraphrase-note">These are machine-written summaries of what each passage says, not the author&rsquo;s words. No quotation on this page is verbatim. Open the source to read the passage itself, and to check its speaker, context and translation.</p><div id="topic-results" role="tabpanel"></div><button id="topic-more" class="rx-button rx-more">Show more authors</button>
 ...
 if(view==='compare'){const sl=[...selected].map(i=>{const a=all[i];return a.s?a:(a.rooms||[]).length===1?a.rooms[0]:null;}),tsl=tslugOf(d.t);
   ...
   const tools=sl.length===2&&sl[0]&&sl[1]?`<p class="rx-compare-links">${deskLink}<a class="rx-text-link" href="/the-faith-received/author/?sh=${encodeURIComponent(sl[0].sh)}#${encodeURIComponent(sl[0].s)}/with/${encodeURIComponent(sl[1].s)}/topic/${encodeURIComponent(tsl)}">Full comparison: every citation between them, all shared topics, Scripture</a> · <a class="rx-text-link" href="/the-faith-received/author/?sh=${encodeURIComponent(sl[0].sh)}&cmp=${encodeURIComponent(sl[1].s)}#${encodeURIComponent(sl[0].s)}/positions/${encodeURIComponent(d.t)}">${esc(sl[0].a)}’s positions with ${esc(sl[1].a)} beside them</a> · <a class="rx-text-link" href="/the-faith-received/connections/#e=${encodeURIComponent(sl[0].s)},${encodeURIComponent(sl[1].s)}">${esc(sl[0].a)} citing ${esc(sl[1].a)} in the citation web</a></p>`:sl.length===1&&sl[0]?`<p class="rx-compare-links">${deskLink}<a class="rx-text-link" href="/the-faith-received/author/?sh=${encodeURIComponent(sl[0].sh)}#${encodeURIComponent(sl[0].s)}/with">Compare ${esc(sl[0].a)} with any author</a> · <a class="rx-text-link" href="${RX.authorURL(sl[0],d.t)}">${esc(sl[0].a)}’s positions on ${esc(d.t)}</a></p>`:'';
 ...
 $('#topic-results').innerHTML=[...Object.keys(RX.shelves),'unknown'].filter(sh=>groups.has(sh)).map(sh=>{const group=groups.get(sh);return `<details class="rx-fold rx-topic-shelf" data-topic-shelf="${sh}"${openedShelves.has(sh)?' open':''}><summary><span><strong>${esc(RX.shelfName(sh)||'Shelf not recorded')}</strong><small>${group.slice(0,2).map(a=>esc(a.a)).join(' · ')}</small></span><span class="rx-fold-count">${fmtR(group.length)} ${group.length===1?'author':'authors'}</span></summary>${pager(RX.topicWindow(group,shelfLimits.get(sh)||0,8),'shelf',sh)}<div class="rx-fold-body rx-pane rx-topic-authors" tabindex="0" role="region" aria-label="${esc(RX.shelfName(sh)||'Unassigned')} authors and statements" data-shelf-pane="${sh}"></div>${pager(RX.topicWindow(group,shelfLimits.get(sh)||0,8),'shelf',sh)}</details>`;}).join('')||'<div class="rx-empty"><h2>No matching passages</h2><p>Try another phrase or reset the filters. This search covers the evidence currently loaded.</p></div>';
 }
...
function rowHTML(r){
  return `<div class="x" style="border-bottom:0;padding:.24rem 0">
    ...
    <a style="font-weight:600" href="/the-faith-received/author/#${aslug(r.a)}">${esc(aName(r.a))}</a> <span class="wk">${esc(tell(r.t))}</span>
  ...
  for(const e of ["E","L","C","H","R","P","M","U"]){
    ...
    const authors=Object.entries(byA).sort((x,y)=>(wOf(y[0])-wOf(x[0]))||(y[1].length-x[1].length)).map(([a2,ars])=>{
      ...
      const body=byW.size>1&&sorted.length>4
        ?[...byW.entries()].sort((x,y)=>y[1].length-x[1].length).map(([w,ws],i)=>`<details class="cd-work vpw"${i===0?' open':''}><summary><span><strong>${esc(tell(ws[0].t))}</strong></span><small class="cd-n">${ws.length}</small>${w?` ${foldOpen(w,ws)}${workSaveBtn(w,ws[0].t,a2)}`:''}</summary>${ws.map(rowH).join('')}</details>`).join('')
        :sorted.map(rowH).join('');
      const head=`<a class="vpa" href="/the-faith-received/author/#${aslug(a2)}">${esc(aName(a2))}</a>${ars.length>1?`<span class="n">${ars.length}</span>`:""}`;
...
async function bookPageAuthor(bslug,c,aslg){
  ...
  page.innerHTML=`
  <div class="crumbs"><a href="/the-faith-received/author/#${aslg}">${esc(ab.a)}</a> · <a href="#b/${bslug}/${c}">${esc(ab.book)} ${c}</a></div>
...
async function compareDesk(host,state,opts={}){
  ...
  const whenOf=au=>(au.r.y?'c. '+au.r.y+' · ':'')+(RX.shelfName(au.sh,au.a)||'');
  ...
  if(dl&&inp){
    ...
    const fill=q=>{const f=RX.fold(q||'');dl.innerHTML=(f?rank(f):rows.slice().sort((a,b)=>(b.w||0)-(a.w||0))).slice(0,30).map(r=>`<option value="${esc(r.a)}">${esc(RX.shelfName(r.sh))} · ${fmtR(r.w)} ${r.w===1?'work':'works'}</option>`).join('');};fill('');
  ...
  const showTopic=t=>{for(const cell of cells.values())cell.io?.disconnect();cur=t;state.sel=t.tslug;emit();const picker=host.querySelector('.cd-topic-picker');if(picker)picker.open=false;const selected=host.querySelector('#cd-selected-topic');if(selected)selected.textContent=t.label;
    ...
    host.querySelector('#cd-topic-links').innerHTML=(t.via.size?`<span class="cd-includes">Includes ${[...t.via].map(esc).join(', ')}.</span> `:'')+(t.reg?`<a class="rx-text-link" href="/the-faith-received/topics/?compare=${authors.map(a=>encodeURIComponent(a.s)).join(',')}#${encodeURIComponent(t.reg)}">All authors on ${esc(t.label)}</a> · <a class="rx-text-link" href="/the-faith-received/connections/#t=${encodeURIComponent(t.reg)}">In the citation web</a>`:'')+(authors.length===2?` · <a class="rx-text-link" href="/the-faith-received/author/?sh=${encodeURIComponent(authors[0].sh)}#${encodeURIComponent(authors[0].s)}/with/${encodeURIComponent(authors[1].s)}">Citations between ${esc(authors[0].a)} and ${esc(authors[1].a)}</a>`:'');
  ...
  function paintMeet(){const box=host.querySelector('#cd-meet-body');if(!box||host.querySelector('#cd-dis')?.hidden||!cur)return;
    ...
    const pairHead=(A,B)=>{const ready=(A.s in recs)&&(B.s in recs);const ab=ready?citesN(recs[A.s],recs[B.s],A,B):null,ba=ready?citesN(recs[B.s],recs[A.s],B,A):null;
      const link=opts.embedded?`<button type="button" class="rx-text-link" data-cd-jump="pair-ab">the citations above</button>`:`<a class="rx-text-link" href="/the-faith-received/author/?sh=${encodeURIComponent(A.sh)}#${encodeURIComponent(A.s)}/with/${encodeURIComponent(B.s)}">every citation between them</a>`;
      return `<small>${ab===null?'counting citations…':`${esc(A.a)} cites ${esc(B.a)} ${fmtR(ab)} · ${esc(B.a)} cites ${esc(A.a)} ${fmtR(ba)}`} · ${link} · <a class="rx-text-link" href="/the-faith-received/connections/#e=${encodeURIComponent(A.s)},${encodeURIComponent(B.s)}">in the citation web</a></small>`;};
...
async function pairPage(slugA,slugB,topicSeg){
  ...
  const block=(from,to,shard,pack,id)=>{
    ...
    return `<section class="rx-pair-block" id="${id}"><h3>${esc(from.a)} on ${esc(to.a)}<small>${fmtR(rows.length)} ${pl(rows.length,'citation')} across ${fmtR(works.length)} ${pl(works.length,'work')} · <a href="/the-faith-received/connections/#e=${encodeURIComponent(from.s||'')},${encodeURIComponent(to.s||'')}">open in the citation web</a></small></h3>
