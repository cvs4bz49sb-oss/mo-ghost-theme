    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: 22, color: '#5f6368',
        }}>
          <svg width="34" height="26" viewBox="0 0 40 30">
            <path d="M2 5 L2 26 L8 26 L8 12 L20 22 L32 12 L32 26 L38 26 L38 5 L34 5 L20 17 L6 5 Z"
              fill="#ea4335" stroke="#ea4335" strokeLinejoin="round" strokeWidth="0.5" />
          </svg>
          <span>Gmail</span>
        </div>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#7e57c2', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 14, fontWeight: 600 }}>
          R
        </div>
      </div>
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div data-mo-gmail-sidebar style={{
          ...
          <button style={{
            ...
            border: 'none', padding: '14px 24px 14px 16px',
            borderRadius: 16, fontSize: 14, fontWeight: 500, cursor: 'default',
            fontFamily: 'inherit',
          }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M22 4H2v16h20V4zM4 8.5l8 5 8-5V18H4V8.5zm8 3L4 6h16l-8 5.5z"/></svg>
            Compose
          </button>
          <div style={{ marginTop: 16 }}>
            {[
              ...
              <div key={item.label} style={{
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="#444746"><path d={item.icon}/></svg>
                <span style={{ flex: 1 }}>{item.label}</span>
                {item.count && <span style={{ fontSize: 12 }}>{item.count}</span>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          {/* Toolbar */}
          <div style={{
            height: 48, borderBottom: '1px solid #e3e6ea',
            display: 'flex', alignItems: 'center', padding: '0 16px', gap: 4,
            color: '#5f6368', fontSize: 13,
          }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style={{ marginRight: 12 }}><path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z"/></svg>
            {['archive', 'report', 'delete'].map((_, i) => (
              <div key={i} style={{ width: 32, height: 32, display: 'grid', placeItems: 'center', borderRadius: '50%' }}>
                <div style={{ width: 18, height: 2, background: '#5f6368' }} />
              </div>
            ))}
            <div style={{ flex: 1 }} />
            <span>1 of 1,284</span>
          </div>
          ...
          <div data-mo-gmail-emailheader style={{ padding: '20px 60px 0', borderBottom: 'none' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
              <div style={{
                ...
                The Weekly Digest · No. {content.issueNumber || '184'} — {content.editorTitle || ''}
                <span style={{ color: '#5f6368', marginLeft: 8 }}>Inbox</span>
              </div>
              <div style={{
                fontSize: 11, fontWeight: 500, color: '#188038',
                border: '1px solid #b8e0c4', padding: '2px 8px', borderRadius: 4,
                marginTop: 6,
              }}>
                Newsletter
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 18, marginBottom: 6 }}>
              <div style={{
                width: 40, height: 40, borderRadius: '50%',
                background: '#ee7d51', color: '#fff',
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, color: '#202124' }}>
                  <span style={{ fontWeight: 500 }}>Mere Orthodoxy</span>{' '}
                  <span style={{ color: '#5f6368' }}>&lt;digest@mereorthodoxy.com&gt;</span>
                </div>
...
// =====================================================
// Mobile preview — simulates a 375px-wide email client by squishing the
// 600px email wrapper inside a 375px stage. Mirrors the @media rules the
// exported HTML uses so all the .mo-* mobile rules fire visually.
// =====================================================
  ...
  return (
    <div data-mo-mobile-outer style={{
      ...
      <div data-mo-mobile-bezel style={{
        ...
        <div className="mo-mobile-stage" style={{
          width: 375,
          margin: '0 auto',
          background: '#fbf7ee',
          borderRadius: 22,
...
// =====================================================
// Raw email viewer — shows the email centered on a soft backdrop
// =====================================================
...
// =====================================================
// Export modal — generate flat HTML for Kit
// =====================================================
function ExportModal({ open, onClose, isMember, accent, density, divider, content }) {
  const [imageMode, setImageMode] = React.useState(() => localStorage.getItem('mo:exportImageMode') || 'auto'); // 'auto' | 'placeholder' | 'datauri'
  const [target, setTarget] = React.useState(() => localStorage.getItem('mo:exportTarget') || 'kit'); // 'kit' | 'generic'
  ...
  const [imageOverrides, setImageOverrides] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('mo:exportImageOverrides') || '{}'); }
    catch { return {}; }
  });
  const [overridesOpen, setOverridesOpen] = React.useState(false);
  const [subject, setSubject] = React.useState(() =>
    `The Weekly Digest — No. ${content.issueNumber || ''} · ${content.subjectLine || content.editorTitle || ''}`.trim()
  );
  ...
  // Re-generate whenever the modal opens or settings change. Debounced so
  // rapid setting changes don't pile up concurrent React renders.
  React.useEffect(() => {
    if (!open) return;
    setGenerating(true);
    setError(null);
    ...
    const handle = setTimeout(async () => {
      try {
        const html = await exportEmailHtml({
          ...
          imageMode, imageBaseUrl, imageOverrides, subject, preheader, target,
        });
      ...
      <div data-mo-modal-shell onClick={(e) => e.stopPropagation()} style={{
        ...
        <div data-mo-modal-body style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          <p style={{
            ...
            Generates a single-file HTML email with all styles inlined, table-based layout, and Outlook-safe markup. {target === 'kit'
              ? <>Upload to Kit as an <strong>Email Template</strong> (Account → Email Templates → New Template). Includes <code style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12 }}>{'{{ message_content }}'}</code>, <code style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12 }}>{'{{ unsubscribe_url }}'}</code>, and <code style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12 }}>{'{{ subscriber_preferences_url }}'}</code> merge tags Kit requires.</>
              : <>Plain HTML — paste into any email client's source/HTML view.</>
            }
          </p>
          ...
          <div style={{ display: 'grid', gap: 16, marginBottom: 20 }}>
            ...
            <div>
              <label style={labelStyle}>Preheader (preview text)</label>
              <input type="text" value={preheader} onChange={(e) => setPreheader(e.target.value)} style={inputStyle} placeholder="Hidden text shown after subject in inbox preview" />
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                {[
                  ...
                  <button key={opt.v} onClick={() => setImageMode(opt.v)} style={{
                    flex: 1,
              ...
              {imageMode === 'auto' && (
                <div style={{
                  ...
                  background: '#fff', border: '1px solid #e6d8be', padding: '10px 12px',
                }}>
                  Article hero images are embedded as data URIs (so they always render). Add overrides below to swap any image for a hosted URL — useful for logos that need to point to your CDN, or to keep file size down.
                </div>
              )}
              {imageMode === 'placeholder' && (
                <>
                  <input type="url" value={imageBaseUrl} onChange={(e) => setImageBaseUrl(e.target.value)} style={inputStyle} placeholder="https://mereorthodoxy.com/wp-content/uploads/digest" />
                  <div style={{
                    fontFamily: '"Source Sans 3", Arial, sans-serif',
                    fontSize: 12, color: '#6b6258', marginTop: 6, lineHeight: 1.5,
                  }}>
                    Each <code style={{ fontFamily: 'ui-monospace, monospace' }}>assets/foo.jpg</code> becomes <code style={{ fontFamily: 'ui-monospace, monospace' }}>{imageBaseUrl}/foo.jpg</code>. Upload the asset files to that location on your site, or paste a different base URL.
                  </div>
                </>
              )}
              {imageMode === 'datauri' && (
                <div style={{
                  fontFamily: '"Source Sans 3", Arial, sans-serif',
                  fontSize: 12, color: '#6b6258', lineHeight: 1.5,
                  background: '#fff4d6', border: '1px solid #e8d4a0', padding: '10px 12px',
                }}>
                  Embeds every image directly inside the HTML. <strong>Warning:</strong> Gmail clips emails over 102KB, and several of the source images are several MB each. This will break Gmail rendering. Use only for testing.
                </div>
              )}

              {/* Per-image URL overrides — available in all modes */}
              <>
                  <button
                    onClick={() => setOverridesOpen(o => !o)}
                    style={{
                      marginTop: 14,
                      background: 'transparent', color: '#2d2927',
                      border: 'none', borderBottom: '1.5px solid #2d2927',
                      padding: '4px 0',
                      fontFamily: '"Source Sans 3", Arial, sans-serif',
                      fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase',
                      cursor: 'pointer', borderRadius: 10,
                  ...
                  {overridesOpen && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        Paste a hosted URL for any image (Kit's image library, your CDN, etc.). Empty = use the base URL above.
                      </div>
                      {listImageFilenames(content).map((fn) => (
                        <div key={fn} style={{ marginBottom: 10 }}>
                          <label style={{
                            display: 'block',
                            fontFamily: 'ui-monospace, Menlo, monospace',
                            fontSize: 11, color: '#2d2927', marginBottom: 4,
                          }}>{fn}</label>
                          <input
                            type="url"
                      ...
                      {Object.values(imageOverrides).some(v => v && v.trim()) && (
                        <button
                          onClick={() => setImageOverrides({})}
                          style={{
                            background: 'transparent', color: '#6b6258',
          ...
          <div style={{
            ...
            background: '#fff', border: '1px solid #e6d8be',
            fontFamily: '"Source Sans 3", Arial, sans-serif', fontSize: 12,
            marginBottom: 16,
          }}>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#9a8773', marginBottom: 2 }}>Size</div>
              <div style={{ color: overGmailLimit ? '#c1593c' : '#2d2927', fontWeight: 600 }}>
                {output ? `${sizeKB} KB` : '—'}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#9a8773', marginBottom: 2 }}>Gmail clip</div>
              <div style={{ color: overGmailLimit ? '#c1593c' : '#3a7d49', fontWeight: 600 }}>
                {overGmailLimit ? 'Over 102KB — will clip' : 'Safe'}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#9a8773', marginBottom: 2 }}>Version</div>
              <div style={{ color: '#2d2927', fontWeight: 600 }}>{isMember ? 'Paid Member' : 'Free Subscriber'}</div>
            </div>
          </div>

          {error && (
            <div style={{
              padding: '10px 14px', background: '#5c2b2e', color: '#ffd3cf',
              fontFamily: '"Source Sans 3", Arial, sans-serif', fontSize: 12,
          ...
          {/* Output preview */}
          <label style={labelStyle}>HTML Output</label>
          <textarea
            readOnly
        ...
        <div data-mo-modal-footer style={{
          ...
          <button
            ...
            style={{
              background: 'transparent', color: '#2d2927',
              border: '1.5px solid #2d2927', padding: '9px 18px',
              fontFamily: '"Source Sans 3", Arial, sans-serif',
              fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase',
          ...
          <button
            ...
            style={{
              background: '#ee7d51', color: '#fff',
              border: '1.5px solid #ee7d51', padding: '9px 18px',
              fontFamily: '"Source Sans 3", Arial, sans-serif',
              fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase',
              cursor: output ? 'pointer' : 'not-allowed',
              borderRadius: 10, opacity: output ? 1 : 0.5,
...
// App
// =====================================================
// Hydrate content from localStorage so an issue you've authored stays put
// across refresh and serves as the next issue's starting point. Falls back
// to DEFAULT_CONTENT for first-time visitors or if storage is corrupt.
...
function App() {
  ...
  // never falls into the gap.
  React.useEffect(() => {
    const handle = setTimeout(() => {
      try { localStorage.setItem('mo:content', JSON.stringify(content)); }
      catch (e) { console.warn('Could not save content', e); }
    }, 300);
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        {tweaks.preview === 'client' ? (
          <GmailChrome content={content}>{email}</GmailChrome>
        ) : tweaks.preview === 'mobile' ? (
          <MobilePreview>{email}</MobilePreview>
        ) : (
          <RawPreview>{email}</RawPreview>
        )}
      </div>
