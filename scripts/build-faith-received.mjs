...
<script src="{{asset "js/faith-indexes.js"}}"></script>`;
  ...
  //
  // These 13 curated topics predate and are unrelated to the mo-tfr R2
  // bucket's own topic layers, which are two different passes over
  // the same ~29,000-work corpus:
  //
  //   v1/mine/topic2-all/{slug}.json — ~190 granular doctrinal topics,
  //   each with every author cited on it ranked by position count and
  //   a sample of quotable, page-located positions. This is the
  //   primary content of the enrichment section (rendered
  //   author-strip-first, positions grouped Fathers/Medieval/
  //   Reformation/Post-Reformation) — see TOPIC2_MAP in
  //   assets/js/faith-topic-synthesis.js, which fetches these live
  //   (small enough per-topic not to pre-bundle).
  //
  //   v1/graph/graph.json — a coarser, earlier 43-locus pass where
  //   each Locus carries one AI-generated prose paragraph. Demoted to
  //   a collapsed "Editorial overview" under the author strip/quotes.
  //   Extracted once into assets/data/faith-received/topic-synthesis.json
  //   (~68 KB of just the 43 Locus nodes) rather than fetched live —
  //   graph.json itself is 12.8 MB. Re-extract that file if the graph
  //   is regenerated.
  //
  // Neither layer is fully surfaced here: topic2-all alone is ~190
  // topics, and forcing all of them into these 13 editorial pages
  // would bury the curation. Each curated topic maps to its 1-2
  // heaviest topic2-all topics and closest locus/loci; the rest get
  // their own browse surface at /the-faith-received/doctrines/ (see
  // custom-faith-doctrines.hbs + assets/js/faith-doctrines.js, which
  // reuses window.MOTopicSynth.renderTopic2Block from this same file).
  ...
  // Emits the hidden mount faith-topic-synthesis.js fills in at
  // runtime. Empty string for a topic with no mapped locus, so no dead
  // markup ships for topics not (yet) covered.
  function topicSynthesisMount(slug) {
    if (!TOPIC_LOCUS_MAP[slug]) return "";
    return `
    <section class="faith-topic-synthesis" data-faith-topic-synthesis data-topic-slug="${slug}" hidden>
      <div class="container faith-doc-inner">
        <div class="faith-topic-synthesis-inner">
          ...
          <div class="faith-topic-synthesis-body" data-faith-topic-synthesis-body></div>
        </div>
      </div>
    </section>`;
  }
...
<script src="{{asset "js/faith-modernize.js"}}"></script>
<script src="{{asset "js/faith-received.js"}}"></script>
<script src="{{asset "js/faith-topic-synthesis.js"}}"></script>`;
    await writeFile(
